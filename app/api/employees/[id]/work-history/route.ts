...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const url = `${SERVER_API_URL}/api/employees/${id}/work-history${queryString ? `?${queryString}` : ''}`
    ...
    if (!response.ok) {
      return NextResponse.json(
        { success: false, message: data.message || 'Failed to fetch employee work history' },
